...
## Policy defaults generated by the wizard

The wizard writes a disabled-by-default `policy` section so a generated configuration is immediately compatible with Policy-as-Code without unexpectedly blocking developer workflows. Advanced mode and answer files can enable `check` or `enforce`; profile overlays may make CI or production stricter than developer profiles. Secrets, approval credentials and private signing keys are never written by the wizard.
