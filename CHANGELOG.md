...
- `rfm runtime status`, `doctor`, `wait` and dependency-ordered `up` workflows.
- Separate container running state, Compose health and service readiness reporting.
- HTTP, TCP and command readiness probes with per-service timeout and remediation.
- Failure diagnostics with optional Compose logs and JSON output for CI.
- Runtime defaults generated by the configuration wizard when Compose services are scanned.

### Changed

- Profile overlays may now override the top-level `runtime` section.
- Selecting one runtime service automatically includes its declared dependencies.
